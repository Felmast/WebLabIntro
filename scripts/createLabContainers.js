		...
		function(response) {
		  // Parse JSON string into object
			...
			newHomework.innerHTML = 
						...
						'<button class="downloadButton" type="submit" onclick="window.open('+"'"+'data/practices/'+json.filename+"'"+')">Ver archivo</button>'+
						'<embed src="data/practices/'+json.filename+'" class="textFile">'
			container.appendChild(newHomework);
		 }, filename);
}

function createLabContainers2(json) {
	...
	newHomework.innerHTML = 
				'<summary class="homeworkSummary">'+ json.name +
						...
						'<button class="downloadButton" type="submit" onclick="window.open('+"'"+'data/labs/'+json.filename+"'"+')">Ver archivo<img src="images/downloadButton.svg" class="smallIcon"></button>'+
						'<embed src="data/labs/'+json.filename+'" class="textFile">'+
						'</div>'
	container.appendChild(newHomework);
}
